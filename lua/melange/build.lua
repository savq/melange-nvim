...
local uv = vim.loop
local palettes = require 'melange.palettes'
...
local function get_palette24(variant)
  local colors = palettes[variant or vim.opt.background:get()]
    -- stylua: ignore
    return {
      bg             = colors.a.bg,
      fg             = colors.a.fg,
...
local function build(terminals)
  for _, bg in ipairs { 'dark', 'light' } do
    ...
    for term_name, term in pairs(terminals) do
      ...
      local dir = get_melange_dir() .. '/term/' .. term_name
    ...
    -- Write JSON files
    fwrite(vim.json.encode(palette), get_melange_dir() .. ('/melange_%s.json'):format(bg))
  end
end
